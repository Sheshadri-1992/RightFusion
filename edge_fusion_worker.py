...
def convert_json_to_pyobj(arg_event_json):
    logging.info("[Edge Fusion Worker] Convert json to pyobj for image objects")
...
def download_item_from_database(arg_event_payload: dict):
    logging.debug("[Edge Fusion Worker] Download the output from the CouchDB server")
    func_context = zmq.Context()
    func_socket = func_context.socket(zmq.REQ)

    # logging.info(container_ip_address, arg_event_payload)
    func_socket.connect("tcp://" + constants.EDGE_CONTROLLER + ":12000")
...
def write_output_to_the_data_service(arg_event_payload):
    logging.debug("[Edge Fusion Worker] Send the output to the CouchDB server")
    func_context = zmq.Context()
    func_socket = func_context.socket(zmq.REQ)
...
class FusionWorker:
    ...
    def __init__(self):
        ...
        Container lists
        '''
        # nsfw_containers = ["rf_pytnsfw1", "rf_pytnsfw2", "rf_pytnsfw3"]
        nsfw_containers = []
        # image_annotation_containers = ["rf_annotation1"]
        image_annotation_containers = []
        # multi_image_resizing_containers = ["rf_multi_ir1"]
        multi_image_resizing_containers = []
        # image_concat_containers = ["rf_image_concat1"]
        image_concat_containers = []
        text_summarization_containers = ["rasp_rf_ts1"]
        profanity_containers = ["rasp_rf_pd1"]
        spam_det_containers = ["rasp_rf_sd1"]
        ...
        for container_name in self.container_list:
            ...
            try:
                ...
                if container_state == "exited":
                    logging.info("[Edge Fusion Worker] Cold start. Starting the container {0}".format(container_name))
                    ...
                    if container_name == "rf_pytnsfw2":
                        ...
                        os.system("docker update {0} --cpus=2 --cpuset-cpus=1,2".format(container_name))
                        self.container_resource_dict[container_name] = 2
                    elif container_name == "rf_pytnsfw3":
                        os.system("docker update {0} --cpus=3 --cpuset-cpus=1,2,3".format(container_name))
    ...
    def select_execution_instance(self, arg_function_name: str, required_resource_spec: dict) -> str:
        ...
        logging.info("[Edge Fusion Worker] Chosen Worker and resource spec {0}".format(container_info))
...
if __name__ == "__main__":
    ...
    print("[Edge Fusion Worker] Service started send requests to tcp://{0}:11000".format(local_ip_address))
