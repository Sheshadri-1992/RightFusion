...
'''
Cloud Server is the Edge Controller
'''

def download_item_from_database(arg_event_payload: dict):
    logging.info("[Fusion Worker] Download the output from the GCP Cloud Emulator Service")
...
def convert_json_to_pyobj(arg_event_json):
    logging.info("[Cloud Server] We are here")
...
def send_request_to_edge_controller(arg_event_payload):
    """
        Download data for the starting function
        """
    arg_event_payload = json.loads(arg_event_payload)
    arg_event_payload["data_op_type"] = "get"
    arg_event_payload["sending_function"] = "cloud_server"
    if arg_event_payload["input_array"] is None or len(arg_event_payload["input_array"]) == 0:
    ...
    cloud_edge_socket.connect("tcp://" + constants.EDGE_CONTROLLER + ":10001")
...
def call_fusion_service(arg_event_payload):
    ...
    # logging.info(container_ip_address, arg_event_payload)
    func_socket.connect("tcp://" + container_ip_address + ":11000")
...
if __name__ == "__main__":
    ...
    my_socket.bind("tcp://" + local_ip_address + ":10000")

    logging.info("[Cloud Server] Service started send requests to tcp://{0}:10000".format(local_ip_address))
    ...
    while True:
        ...
        if request_type == "execute_function":
            if "layer" in request_dict:
                logging.info("[Cloud Server] The layer to which request is sent is {0}".format(request_dict["layer"]))
                if request_dict["layer"] == constants.RASPBERRY:
                    logging.info("[Cloud Server] Forwarding request to Edge Server")
                    send_request_to_edge_controller(exec_payload)
                else:
