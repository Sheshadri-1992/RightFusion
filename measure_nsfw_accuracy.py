...
if __name__ == "__main__":
    ...
    nsfw_pipeline_obj = image_inferencing_predictor.ImageInferencing()
    nsfw_pipeline_obj.new_train_all_models()
    req_count = 1
    new_arr = [55.0, 640, 427, 1, 55.0, 2]
    with open("nsfw_new_new_dataset_prediction.txt",'a') as fd_file:
        # for i in range(0, len(image_index_array)):
        for i in range(0, len(image_array)):
        # for i in range(0, 1):
            index = image_index_array[i]
            image_path = "nsfw_testing_dataset/" + image_array[i]
            ...
            input_size = kb
            prediction_array_input = [input_size, width, height, 1, input_size*1, 1]
            output = nsfw_pipeline_obj.test_predict(prediction_array_input)
            print(output)
            new_dict = {"nsfw" : output["nsfw"]['xeon'][0],
                        "image_annotation" : output["image_annotation"]['xeon'][0],
                        "image_concatenation" : output["image_concatenation"]['xeon'][0],
                        "multi_ir": output["multi_ir"]['xeon'][0],
                        "image_size": int(input_size)}
            print(int(input_size),",", output["nsfw"]['xeon'][0])
